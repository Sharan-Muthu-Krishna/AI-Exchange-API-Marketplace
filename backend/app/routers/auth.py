...
async def setup_admin(
    ...
    Requires the JWT_SECRET_KEY as authentication.
    DELETE THIS ENDPOINT AFTER CREATING ADMIN!
    """
    # Verify using JWT secret as a simple auth
    ...
    result = await db.execute(
        select(User).where(User.email == "admin@aiexchange.com")
    )
    if result.scalar_one_or_none():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    return {"message": "Admin created successfully", "email": "admin@aiexchange.com"}
